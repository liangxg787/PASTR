# -*- coding: UTF-8 -*-
"""
@Time : 11/07/2025 10:53
@Author : Xiaoguang Liang
@File : evaluation_metrics_v2.py
...
from src.metrics.ChamferDistancePytorch.chamfer3D.dist_chamfer_3D import chamfer_3DDist
from src.metrics.ChamferDistancePytorch.fscore import fscore
...
def process_emd_distance(emd_batch):
    row_means = emd_batch.mean(dim=1).detach().cpu()
    sqrt_means = np.sqrt(np.clip(row_means, a_min=0, a_max=None))
    return sqrt_means
...
def EMD_CD(sample_pcs, ref_pcs, batch_size, reduced=True):
    ...
    for b_start in trange(0, N_sample, batch_size):
        ...
        emd_batch, _ = EMD(sample_batch.cuda(), ref_batch.cuda(), EPS, ITERATION)
        emd_batch = process_emd_distance(emd_batch)
    ...
    results = {
        ...
        'fscore': round(fs_lst.cpu().detach().item(), 3)
    }
    return results
...
def _pairwise_EMD_CD_(sample_pcs, ref_pcs, batch_size):
    ...
    for sample_b_start in trange(N_sample):
        ...
        for ref_b_start in range(0, N_ref, batch_size):
            ...
            emd_batch, _ = EMD(sample_batch_exp.cuda(), ref_batch.cuda(), EPS, ITERATION)
            emd_batch = process_emd_distance(emd_batch)
...
def compute_all_metrics(sample_pcs, ref_pcs, batch_size, simple_mode=True):
    if simple_mode:
        return EMD_CD(sample_pcs, ref_pcs, batch_size)
    results = {}
...
if __name__ == "__main__":
    ...
    emd_batch, _ = EMD(x, y, EPS, ITERATION)
    emd_batch = process_emd_distance(emd_batch)
    # dis = np.sqrt(emd_batch.detach().cpu()).mean()
    print(emd_batch.shape)
    print(emd_batch.mean().detach().item())
    # print(dis.item())

    jsd = jsd_between_point_cloud_sets(x.detach().cpu().numpy(), x.detach().cpu().numpy())
    print(jsd)
