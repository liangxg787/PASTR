...
class ShapNetDataset(torch.utils.data.Dataset):
    def __init__(self, data_path, repeat=None, **kwargs):
        ...
        self.img_processor = ImageProcessorV2(**self.hparams.image_processor_cfg.params)
...
class CLIPassoSketchDataset(ShapNetDataset):
    ...
    def __getitem__(self, idx):
        ...
        if mesh_name:
            ...
            # svg = SVG(sketch_path)
            # img = svg_to_img(svg)
            output = self.img_processor(sketch_path, to_tensor=False)
            img = output['image']
            ...
            item = {"latents": latents, "image": img, 'mask': output.get('mask')}
    ...
    def get_mesh_name(self, idx):
        ...
        if isinstance(self.mesh_names, (list, np.ndarray)):
            ...
            if len(self.mesh_names) > 0 and isinstance(self.mesh_names[0], bytes):
                self.mesh_names = [name.decode('utf-8') if isinstance(name, bytes) else str(name) for name in self.mesh_names]
        elif isinstance(self.mesh_names, dict):
            # If the value is bytes, convert it to string
            self.mesh_names = {k: (v.decode('utf-8') if isinstance(v, bytes) else str(v)) if isinstance(v, (bytes, np.bytes_)) else v for k, v in self.mesh_names.items()}
...
if __name__ == "__main__":
    ...
    config = OmegaConf.merge(config_3d, config_sketch, config_test)
